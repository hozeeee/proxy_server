...
const configPath = path.resolve(__dirname, 'clash_config.yaml'); // 替换为实际的配置文件路径
...
const mihomoPath = path.resolve(__dirname, 'mihomo-linux-amd64-v3-alpha-dede56f'); // 替换为实际的clash路径
const clash = execFile(mihomoPath, ['-f', configPath], (error, stdout, stderr) => {
  if (error) {
    console.error(`Error: ${error}`);
    return;
  }
